...
import { 
  ...
  Code, CreditCard, Clock, Truck, Package, UserCheck, 
  Home, Hammer, Mountain, Wrench, HardHat
} from 'lucide-react';
...
const Services = () => {
  const securityServices = [
    ...
    { name: 'Speed Check Systems', icon: <Zap size={32} /> },
  ];

  const softwareServices = [
    { name: 'Custom Software Dev', icon: <Code size={32} /> },
    { name: 'Cashless RFID Apps', icon: <CreditCard size={32} /> },
    { name: 'Biometric Attendance', icon: <Clock size={32} /> },
    { name: 'Vehicle Tracking', icon: <Truck size={32} /> },
    { name: 'Inventory Management', icon: <Package size={32} /> },
      ...
      >
        {items.map((item, index) => (
          <motion.div 
            ...
            className="group bg-white p-6 rounded-xl shadow-sm hover:shadow-xl border border-gray-100 hover:border-[var(--color-tech-blue)]/30 transition-all duration-300 flex flex-col items-center text-center transform hover:-translate-y-1 cursor-pointer relative overflow-hidden"
          >
  ...
  return (
    <section id="services" className="py-20 bg-[var(--color-light-gray)]">
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="mt-16">
          <ServiceCategory title="Security Systems" items={securityServices} colorClass="bg-red-500" />
